...
.provider('terminalConfiguration', function () {
    ...
    var config = function () {
        ...
        me.promptConfiguration = { end: '$ ', user: 'student', separator: ':', path: '~' };
...
.service('promptCreator', [function () {
    var prompt = function (config) {
        ...
        var build = function () {
          var lastPath = _path;
          if(_path.indexOf('/') >= 0) {
            var splitPaths = _path.split('/');
            lastPath = splitPaths.pop();
          }

           me.text = 'prime' + _userPathSeparator + lastPath + ' ' + _user + _promptEnd;
